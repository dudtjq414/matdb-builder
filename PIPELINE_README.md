...
flowchart TD
    ...
    subgraph SETUP["  사전 준비 (최초 1회)  "]
        direction LR
        S1(["pip install notebooklm-mcp-cli"]):::userC
        S2(["nlm login · Google 계정 인증"]):::userC
        S1 --> S2
    end
    ...
    subgraph RUN["  실행  "]
        ...
        R2(["파이프라인 실행해줘 → 7가지 정보 입력"]):::userC
        R1 --> R2
    end

    subgraph PIPE["  자동 파이프라인 (5 ~ 10분)  "]
        direction TD
        P0["Phase 0  딥리서치\n웹 자동 검색 → NotebookLM 소스 추가"]:::nlmC
        P1["Phase 1  쿼리 7개 생성\n연구 도메인 특화 탐색 방향 설계"]:::aiC
        P2["Phase 2  NotebookLM 병렬 탐색\n딥리서치 → 소스 추가 → 쿼리 응답  ×7"]:::nlmC
        P3["Phase 3  데이터 추출\n수치 · 출처 · 연도 · 유형 파싱"]:::aiC
        P4["Phase 4  JSON 저장\n중복 제거 → 지정 파일 저장"]:::outC
        P0 --> P1 --> P2 --> P3 --> P4
    end
